...
## **└ 2. 일부가 일치하지 않는다면, 불일치한 부분을 제외한 직전까지 일치하는 패턴에서 경계를 찾습니다.**
...
## **└ 3. 선택된 경계에서의 접두부, 접미부를 파악한 뒤, 찾으려는 패턴을 접미부의 시작까지 이동시킵니다.**
...
![](https://github.com/ChanhuiSeok/chanhuiseok.github.io/blob/master/assets/img/sample/algo14_10.PNG?raw=true)
 ...
 테이블에서 최대 경계 너비값을 채우면 됩니다. 값을 채우고 나면, **얼마나 이동을 해야 하는지는 아래와 같이 계산합니다.**

![](https://github.com/ChanhuiSeok/chanhuiseok.github.io/blob/master/assets/img/sample/algo14_11.PNG?raw=true)

 이동거리를 왜 이런 방식으로 계산하는지는 **2-2. KMP 알고리즘의 순서**의 그림들을 살펴보면서 대입해 본다면 바로 알 수 있습니다.
...
* **일치하는 패턴의 길이가 3일 때**
  * ABA 에서 경계를 찾으면 B 하나만 나오므로 **이 때의 접두부(접미부) 길이 = 최대 경계 너비값** = 1
* **일치하는 패턴의 길이가 4일 때**
  * ABAC에서 경계를 찾을 수 없으므로 최대 경계 너비값은 0
* **일치하는 패턴의 길이가 5일 때**
  * ABACA에서 경계를 찾으면 BAC 하나만 나오고 이 때 최대 경계 너비값 = 1
* **일치하는 패턴의 길이가 6일 때**
  * ABACAB에서 경계를 찾으면 AC 하나만 나오고 이 때 최대 경계 너비값 = 2
* **일치하는 패턴의 길이가 7일 때**
  * ABACABA에서 경계를 찾으면 
    * 접두부, 접미부가 A 일 때 경계 BACAB
    * 접두부, 접미부가 ABA 일 때 경계 CA
  * 이 둘 중에서 최대 길이의 경계는 BACAB 일 때이며, 이 때 최대 경계 너비값은 = 1
* **일치하는 패턴의 길이가 8일 때**
  * ABACABAA에서 경계를 찾으면 BACABA 하나만 나오고 이 때 최대 경계 너비값 = 1
* **일치하는 패턴의 길이가 9일 때**
...
* 일치하는 패턴의 길이가 7 이므로, 표에서 최대 경계 너비값을 읽습니다. = 1
* 이동 거리는 일치하는 패턴의 길이 - 최대 경계 너비 이므로 7 - 1 = 6칸을 이동합니다.

------

![](https://github.com/ChanhuiSeok/chanhuiseok.github.io/blob/master/assets/img/sample/algo14_14.PNG?raw=true)

* 6칸 이동한 후에는 일치하는 패턴의 길이가 맨 처음 A 하나이므로, 표를 확인한 후 이동거리를 계산하면 1 - 0 = 1칸 이동합니다.

------

![](https://github.com/ChanhuiSeok/chanhuiseok.github.io/blob/master/assets/img/sample/algo14_15.PNG?raw=true)

* 1칸 이동한 후 살펴보니 처음부터 불일치하므로, 1칸 이동합니다.

------

![](https://github.com/ChanhuiSeok/chanhuiseok.github.io/blob/master/assets/img/sample/algo14_16.PNG?raw=true)

* 1칸 이동한 후 살펴보니 모두 일치하므로 패턴 매칭(검색)을 완료합니다.
